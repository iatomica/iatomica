...
const COLUMNS: ColumnDef[] = [
  { status: 'todo', label: '1. Por Iniciar', badgeColor: 'bg-slate-700 text-white' },
  { status: 'in_progress', label: '2. En Progreso', badgeColor: 'bg-purple-600 text-white' },
  { status: 'review', label: '3. Revisión / QA', badgeColor: 'bg-cyan-600 text-white' },
  { status: 'done', label: '4. Completado / Ganado', badgeColor: 'bg-emerald-600 text-white' }
];
    ...
    <div className="space-y-4 text-left">
      ...
      >
        <div className="flex items-center space-x-2.5 w-full sm:w-auto">
          <span className="text-xs font-mono font-bold text-slate-400">Filtrar Ciclo:</span>
          <select
        ...
        <div className="text-[11px] font-mono text-slate-400">
          Mostrando <span className="font-bold text-orange-500">{filteredTickets.length}</span> tarjetas
          activas
        </div>
      </div>
                  ...
                  colTickets.map(ticket => {
                    ...
                    return (
                      <div
                        ...
                        className={`p-4 rounded-xl border transition-all text-left shadow-xs hover:shadow-md cursor-pointer group ${
                          darkMode
                      ...
                      >
                        ...
                        {ticket.crm && (
                          <div className="mt-2.5 p-2 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 space-y-1">
                            <div className="flex items-center justify-between text-[11px] font-bold">
                              <span className="truncate">{ticket.crm.clientName}</span>
                              {ticket.crm.budget && (
                        ...
                        <div className="mt-3 pt-2.5 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-[10px] font-mono text-slate-400">
                          <div className="flex items-center gap-1">
                            <MessageSquareText size={11} className="text-purple-500" />
